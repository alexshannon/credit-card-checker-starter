...
function validateCred(array){
  ...
  for(i = array.length - 2; i >= 0; i-=2){
    let cardDigit = array[i] * 2;
    if(cardDigit > 9){
      multipliedSum += cardDigit - 9
    } if(cardDigit <= 9){
      multipliedSum += cardDigit
    } 
  }
  for(i = array.length - 3; i >= 0; i-=2){
    addedSum += array[i];
  }
  cardSum = addedSum + multipliedSum + array[array.length -1];
  if(cardSum % 10){
    return false
  } else{
    return true
